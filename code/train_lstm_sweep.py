...
def run_lstm(df, cv_dict, fold, bottleneck=10, lr=1e-3, cov_loss_weight=0.0):
    ...
    lstm_encoder_hidden = 100
    lstm_decoder_hidden = lstm_encoder_hidden

    encoder_layer_size = [50, 25]
    decoder_layer_size = encoder_layer_size[::-1]

    model = utils.model_lstm_autoencoder(input_size, lstm_encoder_hidden, lstm_decoder_hidden,
                                         encoder_layer_size, decoder_layer_size,
    ...
    metadata = {'cv_dict': cv_dict, 'lr': lr, 'weight_decay': weight_decay, 'max_epochs': max_epochs,
                ...
                'lstm_encoder_hidden': lstm_encoder_hidden, 'lstm_decoder_hidden': lstm_decoder_hidden,
...
def bottleneck_sweep(df, cv_dict, num_folds):
    ...
    for bottleneck in bottleneck_values:
        ...
        for fold in range(num_folds):
            ...
            res_dict, model = run_lstm(df=df, cv_dict=cv_dict, fold=fold, bottleneck=bottleneck)
            print(' ')

            # Save model
            torch.save(model.state_dict(), f'../models/lstm/bottleneck/lstm_fold{fold}_bottleneck{bottleneck}.pt')

            # Save results on every loop in case early stop
            train_results[f'bottleneck_{bottleneck}'][f'fold_{fold}'] = res_dict
            #Save metadata
            output = open(f'../data/lstm_bottleneck_sweep_results.pkl', 'wb')
...
def learning_rate_sweep(df, cv_dict, num_folds):
    ...
    for lr in lr_values:
        ...
        for fold in range(num_folds):
            ...
            # Run one training instance
            res_dict, model = run_lstm(df=df, cv_dict=cv_dict, fold=fold, lr=lr)
            print(' ')

            # Save model
            torch.save(model.state_dict(), f'../models/lstm/learning_rate/lstm_fold{fold}_lr{lr}.pt')

            # Save results on every loop in case early stop
            train_results[f'lr_{lr}'][f'fold_{fold}'] = res_dict
            #Save metadata
            output = open(f'../data/lstm_lr_sweep_results.pkl', 'wb')
            pickle.dump(train_results, output)
            output.close()
